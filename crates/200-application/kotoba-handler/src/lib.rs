...
mod tests {
    ...
    fn test_handler_error_types() {
        let io_error = HandlerError::Io(std::io::Error::new(std::io::ErrorKind::NotFound, "file not found"));
        assert!(format!("{}", io_error).contains("IO error"));

        let parse_error = HandlerError::Parse("invalid syntax".to_string());
        assert!(format!("{}", parse_error).contains("Parse error"));

        let execution_error = HandlerError::Execution("runtime error".to_string());
        assert!(format!("{}", execution_error).contains("Execution error"));

        let network_error = HandlerError::Network("connection failed".to_string());
        assert!(format!("{}", network_error).contains("Network error"));

        let config_error = HandlerError::Config("invalid config".to_string());
        assert!(format!("{}", config_error).contains("Configuration error"));

        let storage_error = HandlerError::Storage("storage failed".to_string());
        assert!(format!("{}", storage_error).contains("Storage error"));

        let unknown_error = HandlerError::Unknown("unexpected error".to_string());
        assert!(format!("{}", unknown_error).contains("Unknown error"));
    }
    ...
    async fn test_unified_handler_execute() {
        ...
        assert!(response.contains("Kotoba Handler Executed"));
    ...
    fn test_handler_context_serialization() {
        ...
        assert!(json_str.contains("/api/test"));
        ...
        assert_eq!(deserialized.body, Some("test data".to_string()));
    }
    ...
    fn test_handler_metadata_serialization() {
        let metadata = HandlerMetadata {
            ...
            description: "Metadata for testing".to_string(),
    ...
    fn test_handler_capabilities_serialization() {
        let capabilities = HandlerCapabilities {
            ...
            supported_content_types: vec!["text/plain".to_string(), "application/octet-stream".to_string()],
        };
        ...
        assert!(deserialized.supported_content_types.contains(&"text/plain".to_string()));
    }
    ...
    fn test_handler_metadata_clone() {
        let original = HandlerMetadata {
            ...
            description: "Testing clone functionality".to_string(),
    ...
    fn test_handler_metadata_debug() {
        let metadata = HandlerMetadata {
            ...
            description: "Testing debug formatting".to_string(),
    ...
    fn test_handler_capabilities_debug() {
        let capabilities = HandlerCapabilities {
            ...
            supported_content_types: vec!["debug/type".to_string()],
        };
        ...
        assert!(debug_str.contains("debug/type"));
    }

    #[test]
    fn test_handler_error_debug() {
        let error = HandlerError::Config("test config error".to_string());
