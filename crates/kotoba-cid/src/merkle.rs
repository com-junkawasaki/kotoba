...
impl MerkleTreeBuilder {
    ...
    /// 中間ノードを作成
    pub fn create_intermediate(&mut self, left_id: &str, right_id: &str) -> kotoba_core::types::Result<String> {
    ...
    /// ノードを検索
    pub fn find_node(&self, id: &str) -> kotoba_core::types::Result<&MerkleNode> {
        self.nodes.iter()
    ...
    /// ノードをIDで検索（可変参照）
    pub fn find_node_mut(&mut self, id: &str) -> kotoba_core::types::Result<&mut MerkleNode> {
        self.nodes.iter_mut()
            .find(|node| node.id == id)
            .ok_or_else(|| KotobaError::NotFound(format!("Node {} not found", id)))
    }
    ...
    /// Merkleプルーフを生成
    pub fn generate_proof(&self, leaf_id: &str) -> kotoba_core::types::Result<Vec<MerkleNode>> {
