...
| **raw メモリ primitive を deny（T1 部分）**: user safe Kotoba はモジュール内任意オフセットの read/write 不可（自前 heap・string ハンドル・container record を破壊する唯一の経路を封鎖）。メモリは境界尊重の accessor（`bytes-*`/`byte-at`/`str-len`・vector/map prelude）経由のみ。**現行実装（2026-08-03 再実装）**: deny 対象は「アドレスを**逆参照**する op」＝ `mem-byte-at`/`mem-i32-at`/`byte-store!`/`i32-store!` の4つ。`alloc`/`str-ptr`/`bytes-ptr` はアドレスを**作る**だけなので admit のまま（これらまで deny すると host op を呼ぶほぼ全ファイルに例外宣言が付き、marker が無意味になる）。**ただし「逆参照を塞げば作られたアドレスは無害」は誤り**（2026-08-03 に訂正）: `(ptr,len,out-ptr,out-cap)` ABI では **host 側が guest 指定のアドレスに書く**ので、`(clipboard-read 0 65536)` のように**リテラルを渡すだけ**で `alloc` も denied op も使わず data segment を潰せた。guest store を塞いで host store を開けたままにするのは primitive の移動でしかない。`kotoba.wasm-exec/writable-output-window` が出力窓を heap base 以上・allocation high-water 以下に制限して閉じた（8 op 共通: clipboard-read / http-fetch / keychain-read / fs-read / log-read / random-bytes / kgraph-get-objects / kgraph-query）。**未閉鎖**: 生存中の heap object を別の heap object の代わりに指すことは依然可能（bump allocator が個別 extent を記録しないため）。sugar は逆参照 op へ lower されるため検査は**surface 側**で行う（＝旧 ADR の「prelude は免除、user source のみ検査」と同じ意味）。container/wire 実装は `(ns _ {:kotoba/raw-memory <reason>})` で局所化して継続（現在 16 module） | ✅ | **T1 部分** | `kotoba.runtime/raw-memory-problems`, `test/kotoba/raw_memory_test.clj`, `tools/raw_memory_audit.clj`, `kotoba.wasm-exec/writable-output-window`, `test/kotoba/host_write_window_test.clj` |
...
| borrow checker（S2、deterministic drop・no implicit clone） | ⬜ | **T1** | — |
...
- **capability 値渡し（S4b）** — `GraphWriteCap` を値として引数渡し＋ effect↔capability 一致 ＋
  CACAO 動的照合。host-runtime 協調と値モデル拡張を要する大物。
